...
# 2. Run seeds in order: permissions -> providers -> mailgun fields -> demo workspace -> mailgun integration
echo "Applying seeds..."
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -f /docker-entrypoint-initdb.d/seeds/005_seed_mailgun_integration.sql
