signal_file='/Users/michael/Sites/WeProDev/wpd-message-gateway/.specify/signals/plan.ready'; printf '\033[33m⏳ Waiting for Master Agent (plan step) to complete (plan.ready)...\033[0m\n'; while [[ ! -f "$signal_file" ]]; do sleep 3; done; tmux resize-pane -t "$TMUX_PANE" -y 999 2>/dev/null || true; tmux select-pane -t "$TMUX_PANE" 2>/dev/null || true; printf '\033[32m✅ Signal received: plan\033[0m\n\n' &&   while true; do
...
You are assigned to Phase: Tasks.
...
HOW TO WRITE THE FILE (choose one — DO NOT use subagent tools):
  ...
  Option 2 — use run_shell_command with a heredoc:
    mkdir -p '/Users/michael/Sites/WeProDev/wpd-message-gateway/specs/017-refactor-readmefile-technical-writer' && cat > '/Users/michael/Sites/WeProDev/wpd-message-gateway/specs/017-refactor-readmefile-technical-writer/tasks.md' << 'PLAN_EOF'
...
Complete your phase tasks as thoroughly as possible. When you finish, exit immediately." | pbcopy 2>/dev/null || true && printf "\033[32mThe prompt has been copied to your clipboard! Paste it into Cursor.\033[0m\n" && cursor "/Users/michael/Sites/WeProDev/wpd-message-gateway" 2>/dev/null || true
    ...
    if [[ "tasks" == "spec" || "tasks" == "plan" || "tasks" == "tasks" || "tasks" == "impl" || "tasks" == "review" ]]; then
    ...
    if [[ "$decision" == "Approve" ]]; then
      touch "/Users/michael/Sites/WeProDev/wpd-message-gateway/.specify/signals/tasks.ready"
      ...
      tmux send-keys -t "wpd-gateway:0.0" "I have rejected the Tasks phase. Please ask me what needs changing, update the file, and then execute precisely this command in your shell tool to resume the pipeline: touch /Users/michael/Sites/WeProDev/wpd-message-gateway/.specify/signals/tasks.revision.ready" Enter
      
      while [[ ! -f "/Users/michael/Sites/WeProDev/wpd-message-gateway/.specify/signals/tasks.revision.ready" ]]; do
        sleep 3
      done
      rm -f "/Users/michael/Sites/WeProDev/wpd-message-gateway/.specify/signals/tasks.revision.ready"
    fi
  done
