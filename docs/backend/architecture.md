...
| `memory_and_provider` | Stored in memory AND sent through the integration; full payload also persisted in PostgreSQL (`stored_messages`). |
...
GatewayService.SendEmail(ctx, workspaceID, email)
    ...
    │
    └── memory_and_provider → memory inbox + stored_messages + integration send
```
...
- Use `memory_and_provider` to keep a local inbox copy, persist to PostgreSQL, AND send to the real provider
