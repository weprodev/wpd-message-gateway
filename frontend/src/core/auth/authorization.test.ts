...
describe("hasRolePermission", () => {
  it("matches seeded role matrices", () => {
    ...
    expect(hasRolePermission(Role.Viewer, Permission.IntegrationsWrite)).toBe(false)
    expect(hasRolePermission(Role.Viewer, Permission.TemplatesWrite)).toBe(false)
    expect(hasRolePermission(Role.Viewer, Permission.InboxWrite)).toBe(false)
    expect(hasRolePermission(Role.Viewer, Permission.APIKeysWrite)).toBe(false)
  })
})
